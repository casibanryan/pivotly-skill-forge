...
import { CONFIG_KEYS, CONFIG_PATH, DEFAULT_API_BASE_URL, loadConfig, readStored, tokenHint, validateApiBaseUrl, validateBackendPath, validateToken, writeStored, } from "./config.js";
...
const SETUP_HINT = "Collect it by asking the user in conversation; /skill-forge-setup walks through every setting. Nothing needs to go in the environment or into a file by hand.";
...
async function configReport() {
    ...
    if (missing.includes("token"))
        notes.push("No token stored: forge_request and authenticated probes are unavailable.");
    if (missing.includes("backend_path"))
        notes.push("No backend repo path stored: forge_git_state and codebase-mine are unavailable.");
...
server.tool("forge_config_set", "Store one or more skill-forge settings for this developer, collected by asking them in conversation — no environment variables, no file editing. Each value is validated before it is written (the URL is parsed and must be a local/dev host; backend_path must exist and be a git checkout) and takes effect immediately with no restart. The token is written to a private file and is never echoed back. Ask for values one at a time, and only for the settings the current task actually needs.", {
    ...
    if (backend_path !== undefined) {
        ...
        if (!v.ok)
            rejected.push(v.error);
        else if (!(await isGitRepo(v.value))) {
...
/* forge_config_clear                                                  */
/* ------------------------------------------------------------------ */
server.tool("forge_config_clear", "Forget stored skill-forge settings — one key, or all of them. Use it when the developer rotates their token, switches backend checkouts, or wants the machine left clean. Only touches this plugin's own config file; never the backend or the repo.", {
    keys: z
...
/* forge_health                                                        */
/* ------------------------------------------------------------------ */
server.tool("forge_health", "Check whether the developer's Pivotly backend is reachable, which health endpoint answers, whether the stored token is accepted, and whether an OpenAPI/Swagger document is served. Call this first in any api-verify workflow; when a setting is missing it names the one to collect.", {}, async () => {
    const cfg = loadConfig();
...
/* forge_openapi                                                       */
/* ------------------------------------------------------------------ */
server.tool("forge_openapi", "Fetch the OpenAPI/Swagger document the backend serves (auto-discovers the path, or takes one) and return a normalized inventory: every path+method with summary, parameters, request-body schema ref, and response codes. Pass filter to narrow to paths containing a substring (e.g. 'core-data').", {
    ...
    for (const p of candidates) {
        try {
            ...
            if (r.status !== 200 || !doc || !(doc.openapi || doc.swagger || doc.paths))
                continue;
            if (raw)
                return text(doc, TOKEN);
            const inventory = [];
...
/* forge_git_state                                                     */
/* ------------------------------------------------------------------ */
server.tool("forge_git_state", "Report the backend repo's git state: current branch, dirty file count, commits behind origin/main, last commit. Run before codebase-mine so extracted contracts match what's actually deployed. Read-only; never checks out, pulls, or resets.", { fetch: z.boolean().default(true).describe("Run 'git fetch origin main' first so 'behind' is accurate") }, async ({ fetch: doFetchRemote }) => {
    const cfg = loadConfig();
    ...
    const git = async (...args) => (await execFileP("git", ["-C", BACKEND_PATH, ...args], { timeout: 20000 })).stdout.trim();
    try {
    ...
    const reminders = [];
    if (branch !== "main")
        reminders.push(`On branch '${branch}', not 'main'. Ask the user to switch or confirm intentionally mining this branch.`);
    if (behind && behind > 0)
        reminders.push(`Local is ${behind} commit(s) behind origin/main. Ask the user to 'git pull' before mining.`);
    if (dirty > 0)
        reminders.push(`${dirty} uncommitted change(s). Mined contracts may include unmerged work; note this in the skill's provenance.`);
